...
# -*- coding: utf-8 -*-
"""
O Guia Emocional dos Signos — DEMO
Parte 1 completa + página de CTA ao final
"""
...
def make_styles():
    ...
    # ── Estilos da página de CTA (fundo escuro) ──────────────
    s['cta_tag'] = ParagraphStyle('cta_tag',
        fontName='Helvetica-Bold', fontSize=9, leading=13,
        textColor=GOLD, alignment=TA_CENTER, spaceAfter=10,
        letterSpacing=3)

    s['cta_headline'] = ParagraphStyle('cta_headline',
        fontName='Times-Bold', fontSize=30, leading=38,
        textColor=GOLD_LIGHT, alignment=TA_CENTER, spaceAfter=8)

    s['cta_sub'] = ParagraphStyle('cta_sub',
        fontName='Times-Roman', fontSize=14, leading=21,
        textColor=HexColor('#d4c8b0'), alignment=TA_CENTER, spaceAfter=6)

    s['cta_item'] = ParagraphStyle('cta_item',
        fontName='Helvetica-Bold', fontSize=11, leading=18,
        textColor=white, spaceAfter=2, leftIndent=18, firstLineIndent=-18)

    s['cta_item_desc'] = ParagraphStyle('cta_item_desc',
        fontName='Helvetica', fontSize=10, leading=15,
        textColor=HexColor('#b0b0d0'), spaceAfter=8, leftIndent=22)

    s['cta_price_label'] = ParagraphStyle('cta_price_label',
        fontName='Helvetica', fontSize=12, leading=18,
        textColor=HexColor('#d4c8b0'), alignment=TA_CENTER, spaceAfter=4)

    s['cta_price'] = ParagraphStyle('cta_price',
        fontName='Times-Bold', fontSize=46, leading=54,
        textColor=GOLD_LIGHT, alignment=TA_CENTER, spaceAfter=4)

    s['cta_guarantee'] = ParagraphStyle('cta_guarantee',
        fontName='Helvetica', fontSize=10, leading=15,
...
def page_normal(cv, doc):
    ...
    cv.setFillColor(MUTED)
    cv.setFont('Helvetica', 8)
    cv.drawCentredString(PW/2, 0.8*cm, 'O Guia Emocional dos Signos  ·  Prévia Gratuita')
    cv.setFillColor(GOLD)
...
def page_cta(cv, doc):
    """Página de CTA — fundo escuro premium."""
    cv.saveState()
    cv.setFillColor(DARK_BG)
    cv.rect(0, 0, PW, PH, fill=1, stroke=0)
    ...
    # Caixa de destaque para o preço (retângulo dourado semi-transparente)
    cv.setStrokeColor(GOLD)
    cv.setLineWidth(1)
    cv.setFillColor(HexColor('#1a1a2e'))
    cv.roundRect(3.5*cm, 2.8*cm, PW - 7*cm, 4.2*cm, 6, fill=1, stroke=1)
...
# ════════════════════════════════════════════════════════════════
#  CONTEÚDO DA PARTE 1 (idêntico ao ebook completo)
# ════════════════════════════════════════════════════════════════
...
def build_cta():
    """Página de CTA — fundo escuro, texto dourado/branco."""
    story = []

    story.append(Spacer(1, 0.8*cm))
    story.append(Paragraph('PRÉVIA GRATUITA', ST['cta_tag']))
    story.append(Spacer(1, 0.3*cm))
    story.append(Paragraph('Isso foi só uma prévia.', ST['cta_headline']))
    story.append(Spacer(1, 0.2*cm))
    story.append(Paragraph(
        'Você acabou de ler os Fundamentos do Zodíaco.<br/>'
        'O guia completo tem muito mais esperando por você.',
        ST['cta_sub']
    ))
    story.append(Spacer(1, 0.5*cm))

    story.append(HRFlowable(
        width='70%', thickness=0.8, color=GOLD,
        spaceAfter=18, spaceBefore=4
    ))

    itens = [
        ('✦  Os 12 signos em profundidade',
         'Perfil emocional completo, como cada signo ama, seus medos e padrões secretos'),
        ('✦  66 combinações amorosas',
         'O que cada par de signos revela — incluindo o seu'),
        ('✦  Padrões emocionais profundos',
         'Por que você se apaixona sempre pelo mesmo tipo e como sair desse ciclo'),
        ('✦  Capítulo Secreto',
         'O manual de influência psicológica usando a astrologia emocional'),
        ('✦  Bônus exclusivo',
         'Guia de bolso com os perfis emocionais de todos os signos'),
    ]

    for titulo, desc in itens:
        story.append(Paragraph(titulo, ST['cta_item']))
        story.append(Paragraph(desc, ST['cta_item_desc']))
    ...
    # Caixa de preço — posicionada sobre o retângulo desenhado no canvas
    story.append(Spacer(1, 0.3*cm))
    story.append(Paragraph('ACESSO IMEDIATO AO GUIA COMPLETO', ST['cta_tag']))
    story.append(Spacer(1, 0.1*cm))
    story.append(Paragraph('R$39,90', ST['cta_price']))
    story.append(Paragraph(
        'Garantia total de 7 dias  ·  Pagamento 100% seguro  ·  PDF entregue na hora',
        ST['cta_guarantee']
    ))
...
def build_doc(output_path):
    ...
    cta_frame = Frame(2.5*cm, 2.5*cm, PW - 5*cm, PH - 5.5*cm,
                      leftPadding=0.5*cm, rightPadding=0.5*cm)

    doc = BaseDocTemplate(
        output_path, pagesize=A4,
        title='O Guia Emocional dos Signos — Prévia Gratuita',
    ...
    doc.addPageTemplates([
        ...
        PageTemplate(id='CTA',    frames=[cta_frame],    onPage=page_cta),
    ])
    ...
    story.append(Paragraph(
        ...
        ST['cover_subtitle']))
    story.append(Spacer(1, 0.5*cm))
    story.append(Paragraph('PRÉVIA GRATUITA  ·  PARTE 1', ST['cover_badge']))
    ...
    doc.build(story)
    print(f'[OK] Demo gerada: {output_path}')


if __name__ == '__main__':
    out = r'c:\Users\Heitor Melo\Desktop\Grana no Digital\Astrologia\O_Guia_Emocional_dos_Signos_DEMO.pdf'
    build_doc(out)
